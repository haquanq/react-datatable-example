  ...
  return (
    <Dialog.Root modal="trap-focus" open={open} onOpenChange={onOpenChange}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed top-1/2 left-1/2 max-h-[calc(100vh-3rem)] w-fit min-w-[min(100vw-3rem,30rem)] -translate-x-1/2 -translate-y-1/2 overflow-y-scroll rounded-lg bg-white p-6 text-gray-900 outline outline-gray-200 transition-[scale,opacity] duration-150 data-ending-style:scale-90 data-ending-style:opacity-0 data-starting-style:scale-90 data-starting-style:opacity-0">
          <div className="flex max-w-100 flex-col gap-4">
            {title && <Dialog.Title className="text-2xl font-medium">{title}</Dialog.Title>}
